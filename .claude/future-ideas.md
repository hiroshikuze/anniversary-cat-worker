...
Issue #160。毎回ランダム生成される猫とは別に、たまに登場する固定のメインキャラクター2匹を作り、長期ユーザーの愛着を狙う（mofusandのサメにゃん相当）。

- **ニャンばー**: faviconの外観がベース
- **サリー**: GitHubの説明ページの外観がベース
- 2匹ともパーティー帽を被っている。仲がよい設定
- それぞれ別途性格を定義する必要あり（掛け合いをさせる想定）

### 検討した方向性と決定事項（2026-08-11）

既存の`CAT_PERSONAS`/`CAT_PERSONALITIES`と同じ「重み付きpick」パターンには乗せられるが、**外見の再現性**が課題になる。Gemini/Pollinationsはテキストプロンプトのみで画像生成しており、参照画像による見た目の固定は行っていない。テキスト記述だけで「ニャンばー」「サリー」の外見を毎回同じに寄せようとしても、生成のたびに毛柄・体型がブレる可能性が高く、mofusandのサメにゃんのような「一目でそれとわかる版権キャラ」には育ちにくい。

検討した2方向:

1. テキストプロンプトのみで外見を固定しようとする（実装は簡単・既存パターン踏襲だが、日によって微妙に別猫に見えるリスクを許容する）
2. **Geminiに固定リファレンス画像を渡して画風・外見を寄せる（image-to-image的アプローチ）**

→ **2の方向で進める**と決定（ユーザー指示）。視覚的一貫性を優先する。

### 未確定・実装前に詰める必要がある点

- Geminiの画像生成API（`gemini-2.5-flash-image`系）がテキスト+参照画像入力（画像編集/キャラクター一貫性のための画像条件付け）に対応しているかの確認（現行の`tryGemini()`はテキストのみ送信）
- リファレンス画像自体の用意（ニャンばー=favicon画像、サリー=GitHub説明ページの画像を流用できるか、別途描き起こしが必要か）
- リファレンス画像の保存場所（R2 or リポジトリ同梱の静的アセット）
- 登場頻度（ゲストキャラの`GUEST_ANIMAL_PROBABILITY`10%のような定数化を想定。メイン猫の代わりに登場させるのか、ゲストのように追加登場させるのかも未確定）
- 2匹の「掛け合い」をさせる場合、1枚の画像に2キャラクターを一貫した外見で収める必要があり、単体キャラより一貫性維持の難度が上がる
- 性格定義（`CAT_PERSONALITIES`と同様の定数化を想定。2匹それぞれ独立に定義が必要）
- Bot投稿頻度（毎平日）の中でどのくらいの割合で登場させるか、Discord通知にどう反映するか

### 行動履歴フィードバック案（未確定・2026-08-17ブレスト）

キャラクターが登場するたびに「その回どう行動したか」を記録・蓄積し、次回以降の生成プロンプトにフィードバックすることで、外見だけでなく性格・エピソードの一貫性も育てられないかという案。

**保存先の切り分け:**

| データ | 保存先 | 理由 |
| --- | --- | --- |
| 固定リファレンス画像（T字ポーズ等） | GitHub（`frontend/images/`等の静的アセット） | 滅多に変わらない・レビュー可能な形が望ましい。GitHub Pages経由で公開URLになるため、Worker側は新規トークンなしで`fetch()`できる（`/proxy-image`と同じ発想） |
| キャラクターの行動履歴・性格設定 | Cloudflare R2 | 生成のたびに増える動的データ。既存のresearch pool・meta.json等と同じ置き場所 |

**画像生成API側の非対称性（未検証・要確認）:**

- Gemini（`gemini-2.5-flash-image`系）はテキスト+参照画像入力に対応している可能性が高いという認識だが、公式ドキュメント・実機での確認はまだ（サンドボックス制限）
- Pollinations.aiは現行の`_buildPollinationsPrompt()`がテキストのみのtxt2imgで、画像入力を受け付けない。2フェーズレースでPollinationsにフォールバックした場合、参照画像なしでキャラクターの一貫性が崩れる
- 対策案: Geminiがタイムアウト・失敗した場合、R2の履歴更新（下記canonTraits等）はキャンセルしてPollinations採用として扱う（Gemini経由で生成できた回のみ「公式設定」に反映する）

**R2更新方式の検討: 生ログ蓄積 vs AI判断による選定**

最初「生ログをそのまま溜めて直近数件をサンプリング」を提案したが、以下の理由で不十分と判断:

- 生データをそのまま並べても、Gemini呼び出しのたびに「どう解釈するか」が再解釈されてブレる
- 矛盾する記録が溜まっても取捨選択する仕組みがなく、キャラクター性が一貫しにくい

→ **AIに「今回追加・更新すべき設定は何か」を判断させる方式**の方が、狙っている「個性・ストーリー性がにじみ出る」効果に近いと結論。ただし新規API呼び出しはコスト・レイテンシーの追加懸念があるため、**既存の`handleResearch()`のGemini呼び出しに相乗りさせる**（`themeKana`等と同じく「ついでに1フィールド増やす」パターン）ことでこれを回避する案を検討中。

想定するR2データ構造（未確定）:

- `history`: 生ログ（append-only・監査/デバッグ用・件数上限で切り詰め）
- `canonTraits`: AIが選定した「公式設定」リスト（上限件数を決めて古いものと入れ替え・統合するルール。プロンプトに実際に注入するのはこちら）

### 着手タイミング

方向性（2ベース）は確定済み。行動履歴フィードバック案は2026-08-17時点でアイデア段階（未確定）。実装計画は次にこのIssueに着手するタイミングでPlan Modeから開始する。

## fal.aiポーリングをCloudflare Queuesに移行する案（未着手・2026-08）

### 背景

fal.aiアップスケール未完了によるbase64フォールバックの調査中、「`ctx.waitUntil()`の~28秒予算が尽きそうになったら、新しいHTTPリクエストとして自分自身（または別エンドポイント）を呼べば、そちらは独立した新しい予算でスタートするのでは」という案をユーザーから相談された。

### 調査結果

- **`ctx.waitUntil()`の~28秒予算は1回のHTTP invocationに紐づく**。別の新規HTTPリクエストとして呼び出せば、Cloudflare側はそれを完全に別のinvocationとして扱い、独立した新しい予算で実行される（一般的なWorkersの挙動からの推測であり、公式ドキュメントに明言箇所はなし・未検証）
- **正式にサポートされている実現手段はCloudflare Queues**。Queue Consumerハンドラーは壁時間上限**15分**（[公式ドキュメント](https://developers.cloudflare.com/queues/platform/limits/)で確認済み）で、`ctx.waitUntil()`の~28秒とは比較にならないほど余裕がある。**Freeプランでも利用可能**（メッセージ保持期間はFreeが24時間固定・Paidは最大14日という差はある）
- 自前のself-fetch（Workerが自分自身のURLを再度叩く）でも技術的には可能と思われるが、`CLAUDE.md`の「BotはHTTP自己呼び出しをせず直接呼ぶ」という既存の設計判断と同種のリスク（自分のURLのハードコード・レート制限との衝突・無限ループ防止）を抱える。Queuesの方がクリーン

### 想定する実装イメージ（未確定）

- `/suzuri-create`の`ctx.waitUntil()`側でfal.aiのrequest_idをQueueに送信（`env.MY_QUEUE.send()`）
- 別のQueue Consumer Workerが受け取り、15分の余裕を持ってポーリング・R2保存・SUZURI登録まで完遂する
- 現行の`falRequestId`をR2に先行保存する設計（Bug#28周辺）は流用できる

### 現状の代替: `/resume-hires/:id`ですでに同種の発想を実装済み

このプロジェクトはすでに「新しい予算での続き」を`/resume-hires/:id`エンドポイントで実現している。ただしWorker自身が自分を呼ぶのではなく、**フロントエンド（ユーザーのブラウザ）がトリガー**する形（60秒ポーリング後に1回呼ぶ）でself-fetchのリスクを回避している。Queues移行で得られるのは「フロント側の訪問・ポーリングに依存せず、確実に完遂する」という信頼性向上だが、新しいインフラ（Queue作成・Consumer Worker追加）が必要な設計変更のため、本格着手には別途計画が必要。

### 着手タイミング

未定。ユーザーから「今は記録のみ」との指示（2026-08-17）。fal.aiのbase64フォールバック頻度が実運用上問題になるレベルで増える場合に再検討する。

## 集客・マーケティング診断（2026-09）

### 背景

PR #175〜#177（etagバグ修正・SUZURI削除ロールバック）完了後、ユーザーから「露出・導線の強化とセール機能の活用に時間を使うとは具体的に何が考えられるか」と相談を受けた。当初は一般的なSNS施策・トリブン調整を提案したが、ユーザーから「そもそもにゃんむすSUZURIを訪れている人が少なすぎるのでは」と鋭い指摘があり、実データで検証した結果、**この指摘が正しいことが確定した**。

### 実データで確認した現状（2026-09診断時点）

**フロントエンドのUmami（直近30日）**: Visitors 10・Visits 12・Events 17（`lang-switch`/`suzuri-click`/`gallery-click`）。SNS経由でサイトに来る人自体が月10人しかいない。

**Bluesky（`@nyanmusu.bsky.social`・認証不要の公開AT Protocol API `public.api.bsky.app`で確認可能）**:

- フォロワー106・フォロー109・総投稿数620（アカウント作成2024-02-08）
- 直近15投稿のエンゲージメント: いいね計11件・リポスト0件・返信1件（`app.bsky.feed.getAuthorFeed?actor=nyanmusu.bsky.social`）

**Mastodon（`mastodon.social/@nyanmusu`・認証不要の`/api/v1/accounts/lookup?acct=nyanmusu`で確認可能）**:

- フォロワー24・フォロー14・総投稿数836
- 直近15投稿のエンゲージメント: favs計2件・reblog0件
- **`discoverable: null`・`indexable: false`だった**（Mastodonのプロフィールディレクトリ・エクスプローラー・検索結果からアカウントが除外される設定。新規フォロワー獲得の入口が実質閉じていた）

### 診断結果: ボトルネックは価格・セールではなく「露出（リーチ）」

1,456投稿（Bluesky+Mastodon合計）という発信量に対しフォロワー合計130人・エンゲージメントほぼゼロという状態から、**投稿頻度でもSUZURI側の価格・セール訴求でもなく、そもそも見る人がいない段階**であると判断した。ファネルで言うと「SNSインプレッション→サイト訪問（月10人）→SUZURIクリック→購入」のうち、**サイト訪問に到達する以前の最上流（SNS上での到達・発見）がボトルネック**。

見つかった具体的な改善点（優先順位順）:

1. **【対応済み・2026-09】Mastodonの`discoverable`/`indexable`をオン**: Web UI「設定→プライバシーとつながりやすさ」→「Feature me in discovery experiences」「公開投稿を検索できるようにする」にチェックを入れて保存。API再確認で`discoverable: true`・`indexable: true`になったことを確認済み。コストゼロで即実施できる改善だった
2. **【未対応】アカウントの人格が「にゃんバーサリー」に一本化されていない**: 両アカウントとも自己紹介文が「自動生成AIなニャンコな画像をつくったりしてるWebシステム技術者です」で始まり、`#にゃんバーサリー`とは別プロジェクト（AIコンテンツ批評アプリ等）の告知も同一アカウントに混在している。猫・記念日好きが見つけてフォローする理由が伝わりにくい構造
3. **【未対応】能動的なフォロワー獲得行動が必要**: 106+24人という規模で2年以上・1,400件超投稿しても伸びていない＝投稿を続けるだけでは新規フォロワーは増えないフェーズ。既存の猫・記念日好きコミュニティへの能動的な交流（他アカウントとの相互フォロー・関連ハッシュタグでの検索経由の発見等）が必要

### 参考: 調査時に見つけた一般的なSUZURI集客事例（未検証の外部情報）

- トリブンを初期は低め（缶バッジ・アクキー100〜200円、Tシャツ300〜500円程度）に設定し、実績とファンがついてから引き上げる型が複数記事で推奨されている（[original-goods-guide.com](https://original-goods-guide.com/2026/08/21/suzuri-not-selling-marketing-tips/)）
- SUZURI内「ズッキュン」機能を活用し初期反応を集めるとおすすめ欄に載りやすくなる（同上）
- SUZURI商品説明文はURLが自動的にクリッカブルリンク化される仕様を活かし、他媒体への導線として使う事例（[too-long-for-sns.hatenablog.com](https://too-long-for-sns.hatenablog.com/entry/2025/01/11/061223)）
- ただし上記は「一定のアクセスがある前提」の施策であり、**現状のにゃんバーサリーには時期尚早**。上記②③の露出改善が先

### 次のアクション（未着手）

②のアカウント人格一本化・③の能動的交流について、ユーザーの承認を得てから着手する。着手時は`worker/bot.js`のBluesky/Mastodonプロフィール文自体はコード管理外（各SNSのWeb UIで手動設定するもの）のため、コード変更は発生しない見込み。
