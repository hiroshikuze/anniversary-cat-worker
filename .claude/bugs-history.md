...
- **修正**: `updateMetaInR2()`をR2条件付きPUT（`onlyIf: { etagMatches: obj.etag }`。**当初`obj.httpEtag`と実装し2026-09-09に`obj.etag`へ訂正**、詳細は下記追記参照）による楽観的並行性制御+有界リトライ（`maxRetries=5`）に書き換え。競合時（`put()`が`null`）は再取得してマージし直しリトライ。リトライ枯渇時はthrowし、SUZURIマテリアル作成済み（課金対象商品がすでに存在）の呼び出し箇所（右/中央グループ・`/resume-hires`）ではDiscord通知を追加
  - **`httpEtag`と決めた根拠（誤り・2026-09追記で訂正）**: 公式ドキュメント（`developers.cloudflare.com/r2/api/workers/workers-api-reference/`を直接fetch）に実例はなかった。`wrangler dev --local`でのMiniflare実地検証はサンドボックスのネットワーク制約（`put()`単体で`Network connection lost`）により不可だったため、グローバルインストール済みwranglerが依存するMiniflareソース（`.../miniflare/dist/src/workers/r2/bucket.worker.js`）を直接読み、`etagMatches`が`// "If-Match"`とコメントされ（RFC 9110のIf-Matchはクォート付き必須）ていることから`httpEtag`と判断した。**実行検証はできておらず参照実装ソースの読解による判断**（誤っていても`onlyIf`が競合を検出できないだけで新規不具合は生まない設計のつもりだった）
  - **ユーザー環境での追加検証も不可（2026-09追記）**: ユーザーに依頼し（1）WSL bash（workerdバイナリがGLIBC_2.32以降を要求しUbuntuでは起動不可）（2）PowerShellでの`--local`（`put()`単体で`Network connection lost`）（3）実際にCloudflareアカウントへログインし使い捨てR2バケット`etag-verify-test`を新規作成のうえ`--remote`（ローカルエミュレーション非経由・実R2 API使用）の3パターンで試行したが、（3）も含めすべて`put()`単体呼び出しの時点で同一エラーとなった。バケット作成（管理API）は成功しているためランタイムのR2バインディング呼び出し（データプレーン）側のみに問題があると推定されるが特定には至らず、本バグ修正自体とは独立した環境・ツール側の問題と判断し追跡を打ち切った
  - **本番デプロイ後に実際に発覚（2026-09-09追記）**: デプロイ翌日、`/suzuri-create`のcenter/rightグループが`Discord通知「SUZURIマテリアル作成済みだがR2メタ書き込みに失敗」`＋エラーメッセージ`Conditional ETag should not be wrapped in quotes ("e8466577...")`で複数回失敗しているとユーザーから報告があった。`developers.cloudflare.com/r2/api/workers/workers-api-reference/`を改めて直接fetchし、「`httpEtag`はHTTPレスポンスヘッダー用のクォート付き文字列、`etag`は生のハッシュ値で`onlyIf.etagMatches`/`etagDoesNotMatch`にはクォートなしの`etag`を使う」と明記されているのを確認した。**「実行検証はできておらず」と自己申告していた推測が外れていたケース**。`onlyIf: { etagMatches: obj.httpEtag }`を`onlyIf: { etagMatches: obj.etag }`に修正した
  - **見落としの理由**: 上記の`httpEtag`判断はMiniflare参照実装のコメント（`// "If-Match"`）という間接証拠のみに基づいていた。今回`developers.cloudflare.com/r2/api/workers/workers-api-reference/`を同じセッション内で改めて直接fetchしたところ、`etag`と`httpEtag`の使い分けが明記されたセクションが実際には存在しており、当時のfetchで見落としていたか、参照した範囲が不十分だった可能性が高い
  - **テストが検知できなかった理由**: `scripts/test-bot.mjs`の`makeMockBucket()`は`onlyIf.etagMatches`の値を「常にクォート付き文字列同士で比較する」実装だったため、本番コードが誤って`httpEtag`（クォート付き）を渡していても、モック側もクォート付きを期待する実装のまま平仄が合ってしまい、テストは正常系として通過していた。モックが本物のworkerd R2 API（クォート付き`etagMatches`自体を`JSG_REQUIRE`で拒否する）の制約を再現していなかったため、実装のバグとモックのバグが噛み合って隠れた典型例
...
- **教訓（2026-09-09追記）**:「実行検証できないので公式ドキュメント＋参照実装ソースで判断し、その旨を明記する」という誠実な進め方をしても、参照した一次情報自体を読み違える・見落とすリスクは残る。**未検証の判断はデプロイ後に実際のログ・エラーで裏を取るまで「暫定」として扱い**、本番投入後に想定通り動いているかの確認を怠らないことが重要。また、モックはテスト対象コードの誤りに合わせて書くと「実装のバグとモックのバグが噛み合って隠れる」（本件の`httpEtag`同士比較モック）ため、**外部APIの制約（今回なら「クォート付きetagMatchesはAPI側が拒否する」）はモック側にも明示的に再現し、テストで実際に拒否されることを検証する**べきだった
- **`httpEtag`誤用の実害・削除ロールバック追加（2026-09-09追記）**: `obj.etag`修正版のデプロイ前、約12.6時間（2026-09-08 15:07 UTC〜2026-09-09 03:44 UTC）バグが本番に存在し続けたため、この間の`updateMetaInR2()`呼び出しは条件が常に真（`meta.json`は`/generate`成功時に必ず作成済みのため`onlyIf`のクォート付きetagは毎回例外を投げる）となり**発生確率100%**だった。この間ユーザーが同じボット画像の共有ページを複数回訪問し、そのたびに「R2側は未登録」と誤認されて再登録が走ったため、`bot/2026-09-09`の絵1件に対し**孤立マテリアルが8件（center/rightペア×4組）**発生した。ユーザーからの指摘（「今後のロールバック」提案依頼）を受け、`updateMetaInR2()`が最終的に失敗（リトライ枯渇）した場合に`deleteSuzuriMaterial()`で直前作成分を削除する補償処理`_updateMetaOrRollback()`を追加した（詳細は`.claude/rules/architecture.md`の「`updateMetaInR2()`最終失敗時のSUZURIマテリアル削除ロールバック」参照）。これにより将来同種の障害（etag以外の原因でR2書き込みが最終失敗するケースを含む）でも孤立マテリアルの累積を防げる
- **教訓（2026-09-09再追記）**: Discord通知だけの安全網は「検知はできるが実害（孤立の累積）は防げない」。実害を及ぼす失敗パスには通知に加えて**可能な限り補償（ロールバック）を実装し、実害そのものを消す**方が根本対応になる。今回は「通知して人間の手動削除に委ねる」設計を一段進め、削除まで自動化することで、同種の障害が起きても孤立が積み上がらないようにした
- **手動復旧（2026-09-11追記）**: `_updateMetaOrRollback()`導入（2026-09-09 10:01 UTC）より前に発生した8件の孤立マテリアル（`bot/2026-09-09`「重陽の節句」・ロールバック機構がまだ無かったため自動削除されず残存）を、別セッションが`query-worker-logs.mjs`で`POST /materials 完了`ログを時系列相関させて特定した。center/rightペア×4組=8件（`20875043`/`20875044`/`20875276`/`20875280`/`20876425`/`20876426`/`20876430`/`20876431`）に加え、httpEtag修正直後（03:57 UTC）に2訪問者が競合して余剰生成された2件（`20876620`/`20876622`。R2の`materialIds`には記録されているため14日後には自動削除されるが、即時性を優先し前倒しで削除）の計10件をユーザーがSUZURI APIから手動削除した。現在の`products`は`20876621`（缶バッジ/アクキー）・`20876624`（Tシャツ/ステッカー）のみを正しく指しており、他の直近日（09-10以降）への影響はないことも確認済み

### 35. 新規Cron Trigger追加がCloudflareアカウント全体の上限（Workers Free 5本）超過でデプロイ失敗（2026-09）

- **症状**: 月替わり壁紙機能（PR #179）マージ後、`Deploy Worker to Cloudflare`ワークフローがCloudflare APIエラーで失敗した。`npx wrangler deploy`のログ: `A request to the Cloudflare API (/accounts/***/workers/scripts/anniversary-cat-worker/schedules) failed. This account has reached the Workers Free limit of 5 cron triggers per account. [code: 10072]`。Workerスクリプト本体のアップロード自体は成功していた（`Uploaded anniversary-cat-worker`のログあり）が、Cronスケジュール同期のAPIコールのみが失敗した
- **原因**: Cron Triggerの上限はCloudflare **アカウント単位**（Workers Freeは5本/アカウント、Paidは250本）であり、**Worker単位ではない**。月替わり壁紙機能の実装時、`wrangler.toml`の`crons`配列を3本→4本（`"0 3 * * *"`追加）に増やす設計にしたが、これはこのWorker単体の本数のみを見た判断だった。実際にはこのアカウントに他プロジェクト（`yobiko`・`yobiko-staging`）もデプロイされており、それぞれ1本ずつCron Triggerを使用していたため、アカウント全体では「本Worker3本＋他2本＝5本（すでに上限ちょうど）」の状態で、そこへ本Workerの4本目を足したことで6本になり拒否された
- **気づいた経緯**: ユーザーに「マージしたので監視して下さい」と依頼され、GitHub Actionsのワークフロー実行結果を確認したところ`Deploy Worker to Cloudflare`が失敗（`conclusion: failure`）していることに気づいた。失敗ジョブのログを取得し上記エラーメッセージを特定。ユーザーに「Cronが5つとは何を指すか」と問われた際、このWorker単体のダッシュボード画面（スクリーンショット）を見せてもらったところ登録済みCronは3本のみ（4本目`"0 3 * * *"`は登録失敗のため反映されていない）であることが確認でき、「アカウント全体」という説明の根拠が視覚的にも裏付けられた
- **修正**: `yobiko-staging`はステージング環境として本番と同一のCron挙動を維持する必要があるとの理由で削減を見送り、代わりに独立Cron`"0 3 * * *"`の新設自体を取りやめ、月末チェックロジックを既存の`"0 15 * * *"`（リサーチプール生成用の毎日Cron）の`scheduled()`分岐内に`ctx.waitUntil()`で相乗りさせる設計に変更した（`wrangler.toml`の`crons`は3本に戻る）。これに伴い月替わり壁紙の生成時刻はユーザー当初希望の12:00 JST頃から0:00 JSTに変更となった
- **場所**: `wrangler.toml`・`worker/index.js` `scheduled()`・`worker/bot.js` `resolveTargetYearMonth()`のドキュメントコメント
- **教訓**: 新しいCron Triggerを追加する変更をレビュー・実装する際は、対象Worker単体の`crons`配列の本数だけでなく、**同一Cloudflareアカウントにデプロイされている他のWorker/プロジェクトが使用中の本数**も考慮する必要がある。この情報はコードベース内には存在せず（アカウント全体の状態は他プロジェクトのリポジトリ・Cloudflareダッシュボードにしかない）、事前にユーザーへ確認するか、Cloudflareダッシュボードで実際の使用状況を確認しない限り、ローカルの`wrangler deploy --dry-run`では検知できない（ビルドは通り、実際のCronスケジュール同期APIコールでのみ失敗するため）。新規Cron追加時は「このアカウントに他にいくつWorkerがあり、Cronを何本使っているか」を実装前にユーザーに確認する習慣が有効。可能な限り新規Cronを追加せず、既存Cronの分岐内に処理を相乗りさせる設計を優先する

### 36. resvg.wasmをR2から実行時fetchしてコンパイルする設計がCloudflare Workersの動的コード生成禁止に抵触しカレンダー合成が常に失敗（2026-09）

- **症状**: 月替わり壁紙機能（PR #179/#180）デプロイ後、ユーザーが`POST /monthly-wallpaper/regenerate`を手動実行したところ、Bluesky/Mastodonへの投稿自体は成功した（`bskyOk: true, mastoOk: true`）が、レスポンスに`composited: false`が含まれていた。`query-worker-logs.mjs`で実ログを確認したところ、`compositeMonthlyWallpaper失敗、未加工画像にフォールバック: WebAssembly.instantiate(): Wasm code generation disallowed by embedder`という警告が出ており、カレンダー格子・月名バッジ・祝日色分けのオーバーレイ合成が毎回失敗し未加工画像にフォールバックしていた
- **原因**: `worker/svg-render.js`の`ensureResvg(bucket)`が、R2（`IMAGE_BUCKET`）から`fetch()`（実体は`bucket.get()`＋`arrayBuffer()`）で取得した生のWASMバイト列を、実行時に`mod.initWasm(wasmBytes)`（内部で`WebAssembly.instantiate(bytes, imports)`を呼ぶ）でコンパイルしようとしていた。Cloudflare Workersはセキュリティ上（JITスプレー対策）、リクエスト処理中の動的WASMコード生成（生バイト列からの`compile`/`instantiate`）を禁止している。この制約はWorkers公式ドキュメントで一般に知られているが、実装時（このセッションの前半）は「バンドルサイズ節約のためR2に置いて実行時fetchする」という設計だけを検討し、Workers側のこの実行時制約自体を検証していなかった
...
- **症状**: Bug#36修正のデプロイ後、Bluesky実投稿を目視確認したところ2点の見た目の問題が見つかった。(1) 左上の月名バッジが「October」「2026」のみで、事前生成バッジPNG時代の元デザインにあった大きな月番号「10」が表示されていない。(2) カレンダーなし版（署名のみ）の画像で、被写体（猫）が画面上部に寄っており、下部に不自然な空白が残っていた。ユーザーから両方とも指摘を受けて発覚した
- **原因**:
  - (1) Satori/resvgへの書き換え（`worker/image-utils.js` `_buildCalendarOverlayElement()`の`monthBadge`）で、元のバッジPNGデザイン仕様（「October」＋大きな「10」）のうち月番号の要素を実装し忘れていた
  - (2) `compositeMonthlyWallpaper()`はカレンダーあり版・なし版とも同一のcover-crop画像（`baseImg`/`baseBytes`）を共有していた。`reserveCalendarSpace`のプロンプト指示（`_buildGeminiPrompt()`/`_buildPollinationsPrompt()`）が画像下部に余白（実測30〜46%）を空けさせるため、カレンダーあり版はその余白をカレンダー帯（半透明パネル）で覆えるが、カレンダーなし版は覆うものがなく被写体の位置がそのまま見えてしまっていた
- **気づいた経緯**: ユーザーがBluesky投稿のスクリーンショットを共有し、「カレンダー無し版について画像を中央にするという話ではありませんでしたか？またOctober 2026について10の文字がなくなっていませんか？」と指摘。(2)については「被写体を上下中央に配置してください、下を開けるのも不自然です」と追加で意図を確認した
- **修正**:
  - (1) `monthBadge`を横並び（`flexDirection: "row"`）に変更し、大きな月番号（`String(month)`）を左に、月名＋年を縦積みブロックにして右に配置
  - (2) 既存の`_detectCropBox()`（`/generate`の自動トリミング機能・`autoCropImage()`と共通のWASM非依存ロジック）を再利用し、カレンダーなし版限定で被写体のバウンディングボックスを検出→検出領域を切り出して同じcover-fitアルゴリズムで1080×1920に再フィット（ズームイン＋再センタリング）する処理を追加した。`maxMarginRatio`は`/generate`用デフォルトの`0.2`ではなく`0.5`を指定（実測30〜46%の余白量を正しく検出するため）。検出できない・失敗時は元のクロップにフォールバックし、カレンダーあり版の生成には影響させない
- **場所**: `worker/image-utils.js` `_buildCalendarOverlayElement()`・`compositeMonthlyWallpaper()`
- **教訓**: 事前に作成した設計仕様（元のバッジPNGデザイン案の「October」＋「10」の2要素構成）が、その後の実装方式の転換（ビットマップ→Satoriベクター）の過程で一部要素が抜け落ちても、ユニットテストが「3要素を持つ」のような粗い構造チェックしかしていないと検知できない（既存テストは`el.props.children.length === 3`のみで、バッジ内部に月番号が含まれるかは検証していなかった）。**視覚的な完成度に関わる要素は、実際に生成された画像をユーザーが目視確認するまで検知できない場合がある**ことを前提に、レイアウト変更を伴う実装ではモックのユニットテストに加えて実機での目視確認を早い段階で依頼する。また、カレンダーあり版専用に設計された要素（プロンプトの余白確保指示等）を「カレンダーなし版」にもそのまま流用する際は、その要素がなし版でも意味を持つか（この場合は「余白を隠すもの」がなし版には存在しない）を個別に検討する必要がある

**追記（2026-09・同日中に発覚した第3の障害）**: 上記(2)の修正（検出領域を切り出して`coverCrop()`で1080×1920へズームイン再フィットする方式）をデプロイし、ユーザーに`POST /monthly-wallpaper/regenerate`の再実行を依頼したところ、`error 1102`が返りDiscord通知も届かなかった。

- **症状**: `error 1102`（Cloudflareの公式エラーコード。Workerがリクエスト処理中にCPU時間またはメモリの上限を超過し、基盤側に強制終了されたことを示す）がcurlの応答として返り、`POST /monthly-wallpaper/regenerate`のレスポンス自体（JSON）が得られなかった。Discord通知（1通目・2通目とも）も一切送信されなかった
- **原因**: `query-worker-logs.mjs`で実ログを確認すると、`[monthly-wallpaper] ... generate 完了 source=gemini`ログの直後に`[error]`（リクエストログ行のエラーマーキングのみで具体的なエラーメッセージなし）が記録され、それ以降のログ（`カレンダー合成失敗`警告や`R2保存完了`等）が一切出力されていなかった。これはJS例外（`try/catch`で捕捉されコンソールにメッセージが出る）ではなく、Cloudflareプラットフォーム自体がリクエストを強制終了した際の典型的なログパターンである。原因は、Satori×2レンダリング＋resvgラスタライズ×2＋Photon合成×2という元々CPU負荷の高い処理に、直前の修正（(2)のズームイン方式）が**フル解像度の追加Lanczos3リサイズ**を丸ごと1回上乗せしたため、CPU予算（Workers Freeの公式上限10msだが、Bug#32で確認された非公式な猶予〔243〜297ms実績〕を含めても）を超過したためと判断した
- **気づいた経緯**: ユーザーから「curlコマンド実行後このエラーが出てDiscordも更新されません」と報告を受け、`query-worker-logs`ワークフローを再度手動発火して実ログを確認した
- **修正**: ズームイン方式（検出領域の切り出し＋`coverCrop()`での再フィット＝追加のLanczos3リサイズを伴う）を廃止し、**リサイズを伴わない「同一スケール内でのシフトのみ」**の軽量な再センタリングに変更した。`baseImg`を作る手前のスケール済み画像（`scaledImg`）を解放せず保持しておき、検出した被写体の垂直中心に応じて`scaledImg`内のクロップ開始位置（`y1`）をずらして`crop()`し直すだけにする（`resize()`の追加呼び出しをゼロにした）。`scaledImg`に十分な垂直方向の余剰がない場合は効果が限定的というトレードオフがあるが、CPU予算超過でリクエストごと失敗する（Discord通知すら届かない）よりは実害が小さいと判断した
- **場所**: `worker/image-utils.js` `compositeMonthlyWallpaper()`
- **教訓**: Cloudflare Workersの`error 1102`はCPU/メモリ上限超過による強制終了を示す公式エラーコードであり、ログが特定の処理ステップの直後で不自然に途切れる（後続の`try/catch`のログすら出ない）ことがこの障害の典型的な兆候である。画像処理（とくにWASMベースのSatori/resvg・Photonの高品質リサイズ）を追加する際は、「機能として正しく動くか」だけでなく「既存の重い処理に対してどれだけCPUコストを上乗せするか」を常に意識する必要がある。とくに`resize()`（とくにLanczos3等の高品質フィルター）は`crop()`単体より大幅にコストが高いと想定されるため、同じ視覚的目的を達成できるなら追加のリサイズを避け、既に計算済みのスケール済み画像を再利用する（クロップ窓をずらすだけ）方式を優先する。CPU予算がすでに逼迫している処理パス（Satori/resvgを含む月替わり壁紙等）に機能追加する際は、`wrangler deploy --dry-run`のビルド確認だけでは不十分で、**実機での実行時間・CPU超過リスクも設計段階で検討する**

**追記（2026-09・シフト方式への軽量化後も1102が断続的に再発）**: シフト方式（`resize()`追加なし）への修正をデプロイし、ユーザーが`POST /monthly-wallpaper/regenerate`を2回連続で手動実行したところ、**1回目は`error 1102`が再発、2回目は`composited: true`で完走**した。

- **症状**: 同一条件下での連続実行で成否が分かれる、不安定な`error 1102`
- **原因**: `query-worker-logs.mjs`で1回目の失敗ログを確認すると、`再センタリング判定完了`の直後・`オーバーレイ描画完了`の手前で途切れていた。シフト方式への変更は「ベースクロップ＋再センタリング」区間の追加コストを削減したが、**真のCPUボトルネックは元から手を付けていなかった`Promise.all([applyOverlay(カレンダー版), applyOverlay(カレンダーなし版)])`区間**（Satori×2＋resvgラスタライズ×2＋Photon合成×2）にあり、生成される画像の内容によって処理時間がばらつき、CPU予算の境界線上で成否が分かれる状態が続いていた
- **気づいた経緯**: ユーザーに2回連続実行を依頼したところ「1回目は従来と同じエラーでした」と報告を受け、ログを相関させて1回目の失敗地点を特定した
- **修正**: カレンダー版は日付ごとの色分け（日曜/祝日=赤・土曜=青）とカスタムフォントが必須のためSatori/resvgを維持する一方、**カレンダーなし版（署名のみ）はSatoriの表現力を必要としないため、Satori/resvg呼び出しを完全に廃止**し、既存のPhoton `draw_text_with_border()`（縁取り文字描画、Robotoフォント固定だがカレンダーなし版はテキスト1行のみのため十分）に置き換えた。Satori render・resvgラスタライズ・オーバーレイ用のPhoton合成を1セット分（全体の約半分）削減した。詳細は`.claude/rules/architecture.md`の「カレンダーなし版のSatori/resvg排除」参照
- **副次的に発覚した別の実装ミス**: 上記の調査中、`_buildSignatureOnlyElement()`の署名テキストに半透明黒背景パネル（`backgroundColor: "rgba(0,0,0,0.35)"`）が描画されていることにユーザーから「署名テキストは黒背景にしなくて良いと伝えていたはず」と指摘を受けた。この黒背景は本来不要という指定を実装時に見落としていたと判明し、背景パネルを削除した（カレンダー版に埋め込まれる署名にも影響するため、カレンダー版・カレンダーなし版の両方で背景なしになる）
- **場所**: `worker/image-utils.js` `_buildSignatureOnlyElement()`・`compositeMonthlyWallpaper()`・`ensurePhoton()`（`draw_text_with_border`を`_photonFns`に追加）
- **教訓**: CPU予算超過の障害は「どこが重いか」を推測だけで判断せず、失敗ログの**途切れた位置**から特定範囲を絞り込む必要がある（今回は2段階の絞り込みで真因＝オーバーレイ描画区間にたどり着いた）。また、断続的にしか再現しない障害（今回は2回に1回）は、1回の成功だけで「直った」と判断せず、複数回の再現テストを依頼する価値がある。ユーザー指摘（黒背景の件）のように、過去の仕様指定が実装に正しく反映されているかは、コードレビューや別の議論の過程で偶然発覚することがあるため、ドキュメント化されていない口頭の仕様変更・指定は都度`.claude/rules/architecture.md`等に明文化しておくことが望ましい

### 38. 月替わり壁紙のオーバーレイ（月名バッジ・カレンダー帯・署名）がスマートフォン実機で画面端に見切れる（2026-09）

- **症状**: 実際に投稿された壁紙画像をユーザーがiPhone 17 Proの待受に設定したところ、左上の月名バッジ・下部のカレンダー帯・その下の署名（© nyanmusu）が、画面の曲面（ディスプレイ端の湾曲）やシステムUI（時計・ホームインジケーター等）に隠れて見切れていた
- **原因**: `worker/image-utils.js`のオーバーレイ配置（`monthBadge`の`top: 56`・`calendarPanel`の`left/right: 40, bottom: 64`・署名の`left: 32, bottom: 32`）は固定px値で決め打ちされており、Cloudflare Workers上での画像合成としては正しく動作するものの、実際のスマートフォン端末の画面形状（画面端の曲面・システムUIによる隠れ）を考慮していなかった。ローカル環境・ユニットテストでは検知できない、実機の物理的な表示条件に起因する問題
- **気づいた経緯**: ユーザーが実際に生成済みの壁紙をiPhoneの待受に設定して目視確認し、「10 Octoberのブロックは160x260ぐらいに配置した方がよさそう」「(c) nyanmusuのx座標を25の下に合わせたほうがよさそう」とフィードバックした。数値の意味（サイズかレイアウト位置か）が曖昧だったため`AskUserQuestion`で確認を試みたが、ユーザーは両方とも回答せず一旦保留。後日「カレンダーパネルの左端に合わせるです、こちらもiPhone 17 Proの曲線部分に隠れていました」と署名位置の意図を明確化し、続けてGeminiとの壁打ちを経た数値仕様（上下セーフエリア・カレンダー帯の横幅と左右マージン）を提示した
- **修正**: `worker/image-utils.js`に`SAFE_AREA_RATIO`（全高の約9%・上下セーフエリア）・`CALENDAR_MARGIN_RATIO`（全幅の約12.25%・カレンダー帯の左右マージン、横幅約75.5%に相当）を導入し、`monthBadge`・`calendarPanel`・署名（Satori版・Photon版の両方）の位置をこれらの比率から算出するよう変更した。3要素が同じ比率定数を参照するため、月名バッジ・署名の左端はカレンダー帯の左端と自動的に揃う。詳細は`.claude/rules/architecture.md`の「スマートフォン実機でのセーフエリア調整」参照
- **場所**: `worker/image-utils.js` `_buildSignatureOnlyElement()`・`_buildCalendarOverlayElement()`・`compositeMonthlyWallpaper()`の`drawSignature()`
- **教訓**: `wrangler deploy --dry-run`やCloudflare上での合成成功（`composited: true`）は「画像処理として正しく動いたか」しか検証できず、「実際のスマートフォン画面でどう見えるか」（画面端の曲面・OSのシステムUIによる隠れ）は実機での目視確認でしか発見できない。オーバーレイ配置をpx固定値で決め打ちする設計は、キャンバスサイズが同じでも実機の表示条件次第で見切れうるため、安全マージンは比率（画面サイズに対する割合）で管理し、根拠となる比率をコード内・ドキュメントの両方に明記しておくとよい。また、ユーザーからの数値フィードバックが曖昧なときに`AskUserQuestion`で確認し、ユーザーが選択肢に回答せず保留した場合は、それを「一旦停止・次の指示を待つ」シグナルとして扱い、憶測で実装を進めない（後日ユーザーが別の言い回しで意図を明確化した）

**追記（2026-09・Bug#38対応PR後、error 1102が別の形で再発→解消）**: Bug#38のデプロイ後、`POST /monthly-wallpaper/regenerate`を実行すると`error 1102`（CPU/メモリ上限超過）が発生した。
...
- **症状**: 実際に投稿された壁紙画像（カレンダー版）をユーザーが確認したところ、署名テキストが見当たらなかった。カレンダーなし版でも確認したところ、署名の位置に文字ではなく黒い塊が表示されていた
- **原因（2つの独立した問題が重なっていた）**:
  1. **カレンダー版（Satori描画）**: Bug#37「署名の黒背景パネルは削除」で半透明黒背景パネルを削除した際、白文字のみ（縁取り・影なし）が残った。明るい背景色（このアプリの猫イラストは`Style: ... white background ...`というプロンプト指示があり白背景が基本）に白文字が重なり、完全に溶けて見えなくなっていた
  2. **カレンダーなし版（Photon `draw_text_with_border()`）**: 本セッションでPhotonの実際のWASM（`@silvia-odwyer/photon`）をNode.js上でローカルに動かし、実際の描画結果を画像として確認する調査を行った。その結果、`draw_text_with_border()`の「縁取り」は文字の輪郭に沿ったストロークではなく、**文字から少しずれた位置に描かれる黒い塗りつぶし矩形**という実装上の癖（バグに近い挙動）だと判明した。Bug#38対応でフォントサイズを13px相当まで縮小していたため、この矩形が文字を覆い尽くし、黒い塊にしか見えなくなっていた。「©」記号のグリフ欠落によるtofu表示という仮説も検証したが、「©」を除いても症状が再現したため否定した
- **気づいた経緯**: ユーザーが実際の投稿画像のスクリーンショットを共有し、「copyright nyanmusuの文字を入れていますか？」「カレンダー無し版も文字が認識できません」と指摘。フォントサイズと縁取り太さの相関を疑ったが、確証を得るためPhotonのWASMを直接ローカルで動かして複数のフォントサイズ・テキストパターンで実際の出力画像を生成・目視確認する実験を行い、上記の縁取り矩形の癖を特定した
- **修正**: Photonの文字描画API（`draw_text()`＝色固定（白）で縁取りなし、`draw_text_with_border()`＝縁取りに上記の癖あり）はどちらも見た目を自由に制御できないという構造的な制約に直面した。「© nyanmusu」は内容が変化しない固定テキストであり、カレンダー本体（日付・月名）をSatori動的描画にした理由（内容が変わるものを事前生成PNGにすると柔軟性を失う）は当てはまらないと判断し、**署名だけを事前生成の透過PNGアセット（`worker/assets/signature.png`）として1回だけデザインし、実行時は既存の`watermark()`で貼り付けるだけにする**方式に変更した。デザイン（単色→縁取り→ドロップシャドウ）はユーザーとPython/PILでのシミュレーションを繰り返して決定した（最終的に柔らかいドロップシャドウを採用）。詳細は`.claude/rules/architecture.md`の「署名を事前生成PNGアセット化」参照
- **場所**: `worker/image-utils.js`（`_buildSignatureOnlyElement()`廃止・`_buildCalendarOverlayElement()`・`compositeMonthlyWallpaper()`）、`worker/assets/signature.png`（新規）、`wrangler.toml`（`.png`のData型ルール追加）
- **教訓**: サードパーティ・依存ライブラリ（今回はPhoton）の挙動について「ドキュメントに書かれている通りだろう」「名前から推測される動作のはずだ」と仮定して議論を進めるのではなく、**実際にその環境（今回はNode.js＋実WASM）で動かして生成物を目視確認する**ことで、ドキュメントだけでは分からない実装上の癖・バグを短時間で特定できた。とくに「文字を画像に焼き込む」系のAPIは色・縁取り・フォントの制御可否がライブラリごとに大きく異なるため、見た目に関する要求（読みやすさ等）が出た時点で、まず対象APIの実際の出力を確認するのが近道である。また、内容が固定（変化しない）テキストに対しては、内容が変化するもの向けに選んだ技術的制約（動的レンダリング）を無条件に引き継ぐ必要はなく、事前生成という選択肢を都度検討する価値がある

**追記（2026-09・デプロイ後の実機検証でさらに別のクラッシュを検知・修正済み）**: 上記修正（PR #189）デプロイ後、ユーザーが`POST /monthly-wallpaper/regenerate`を実行したところ、カレンダー版・カレンダーなし版とも**署名だけでなくカレンダー帯・月名バッジまで一切合成されず、生成イラストのみ**が返ってきた（`composited: false`へのフォールバック）。

- **調査**: `query-worker-logs.mjs`（`--grep monthly-wallpaper --since 6h`）で実ログを確認したところ、`再センタリング判定完了`の直後で`[monthly-wallpaper] カレンダー合成失敗、未加工画像で継続` → `compositeMonthlyWallpaper失敗、未加工画像にフォールバック: unreachable`というログが出ていた。`unreachable`はJS例外ではなくWASM側のtrap（RustパニックがwasmでUNREACHABLE命令にコンパイルされたもの）であることを示すエラーメッセージで、`try/catch`で拾えてはいるがWASM内部が不正な状態で停止したことを意味する
- **原因**: `ensureSignatureAsset()`が`import("./assets/signature.png")`（wrangler.tomlの`.png` Dataモジュールルール）で取得したデフォルトエクスポートは**ArrayBuffer**だった。これをそのまま`_signatureAsset`に保持し、`compositeMonthlyWallpaper()`側で`PhotonImage.new_from_byteslice(signatureBytes)`に渡していたが、**Photon（`@silvia-odwyer/photon`のwasm-bindgenグルーコード）の`new_from_byteslice()`はUint8Arrayを要求しており、ArrayBufferを直接渡すとメモリ参照がずれて不正なバイト列としてPNGデコードを試み、WASM内部で`unreachable`トラップに落ちる**。生成画像本体（`base64ToBytes()`が返すUint8Array）・Satoriのオーバーレイ出力（`renderElementToPngFn()`が返すUint8Array）はいずれも元からUint8Arrayだったため今回はじめて顕在化した型の不一致だった
- **修正**: `ensureSignatureAsset()`内で`new Uint8Array(signaturePng)`に変換してから`_signatureAsset`に保持するよう修正（`worker/image-utils.js`）。ユニットテストの`_setSignatureAssetForTest()`はモックとしてUint8Arrayを直接渡していたため、この型不一致はローカルテストでは検出できなかった
- **場所**: `worker/image-utils.js` `ensureSignatureAsset()`
- **教訓**: Cloudflare WorkersのData モジュールimport（`.ttf`・`.png`等、`wrangler.toml`の`[[rules]] type = "Data"`）は**ArrayBuffer**を返す。Satoriの`options.fonts[].data`はArrayBufferをそのまま受け付けるためBug#39本体では問題が起きなかったが、Photonの`new_from_byteslice()`は明示的にUint8Arrayを要求する。同じ「Dataモジュールから読み込んだバイト列」でも渡し先のAPIによって期待される型が異なるため、新しいバイナリアセットをWASMライブラリに渡す際は毎回そのAPIが要求する具体的な型（ArrayBuffer / Uint8Array / Buffer等）を確認する。今回はローカルユニットテストがモック経由で実際の型不一致を再現できておらず、実機（デプロイ後の本番Worker）ではじめて検知された。この種の「ビルドツール固有のimport挙動」はNode.jsのユニットテストでは検証しきれない領域であり、`wrangler deploy --dry-run`はビルド成功を確認できても実行時の型不一致までは検出できない点に注意

### 40. ダッシュボードの手動Scheduled送信が監査ログ・Cronイベントログいずれにも残らず、想定外の本番投稿が発生し実行者を特定できなかった（2026-09）

- **症状**: ユーザーから「身に覚えのないタイミングでcronが動いていた」と報告。Discord通知には9/24 20:41 JST（11:41 UTC）にBluesky/Mastodonへの投稿完了が記録されていたが、その時刻に発火する設定のCronは存在しなかった
- **調査**: `query-worker-logs.mjs`で実ログを確認したところ、9/24には2回`runBot()`が実行されていた。1回目（22:00 UTC・7:00 JST）は正規のCron（`0 22 * * 1-5`）による正常な実行。2回目（11:41 UTC）は`bot/2026-09-24-2`という2番目のR2スロットに保存されており、既存のCronパターン（すべて「毎時0分」固定）のいずれにも一致しない時刻だった。GitHub Actions・git履歴からも、直近のデプロイ（27時間以上前）や関連する操作が原因でないことを確認した
  - Cloudflareダッシュボードの「Cronイベント」ログ（過去1週間の実行履歴）を確認したところ、9/24分は`0 15 * * *`・`0 16 * * *`の2件のみで、11:41 UTCのエントリは一切存在しなかった。Cronスケジューラーが実際に発火させたイベントは必ずこのログに記録されるため、その不在は「Cronスケジューラーを経由していない」ことの直接証拠となった
  - Cloudflareの監査ログ（Audit Log）API（`/accounts/{account_id}/audit_logs`）を降順50件取得しても9/24のエントリは1件もなく、最新の記録は前日08:22 UTC（デプロイ操作）だった。ログインイベントも含まれていなかったが、これはセッショントークン継続中は再ログインが発生せず記録されないため、と考えられる
- **原因**: `worker/index.js`の`scheduled()`が、`event.cron === "0 15 * * *"`・`"0 16 * * *"`のいずれにも一致しない`event.cron`をすべて無条件でBot Cron分岐（実投稿）に流す設計（`if`によるガードなしの実質`else`）だった。Cloudflareダッシュボードのコード編集画面「HTTP」→「Scheduled」→「送信」は`scheduled()`を直接呼び出す一回限りのテスト機能で、Cronスケジューラーを経由しないためCronイベントログに記録されず、かつ状態変更を伴わない実行のためWorkers操作としても監査ログに記録されない（＝完全に無記名で本番投稿を発火できる）。ユーザー自身に心当たりがなく、実行者・実行経路（ダッシュボードの誤操作か、共有アクセス権を持つ第三者か）は最終的に特定できなかった
- **修正**: 2点の再発防止を実施
  1. `scheduled()`のBot Cron分岐に`event.cron === "0 22 * * 1-5"`の明示一致チェックを追加。一致しない値では投稿せず`console.warn`のみ出力する（多層防御。ダッシュボードで正しい値を明示選択された場合までは防げない）
