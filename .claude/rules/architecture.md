...
| Umamiアクセス解析 | `cloud.umami.is/script.js`を読み込み済み。SUZURIボタンクリックを`data-umami-event="suzuri-click"`＋`data-umami-event-slug={slug}`で計測。Umamiダッシュボードの「Events」タブでスラッグ別クリック数を確認できる |
