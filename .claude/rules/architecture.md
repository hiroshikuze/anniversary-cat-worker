...
#### 本格対応に必要な非同期アーキテクチャ（ctx.waitUntil()方式）

```text
【フロントエンドの2並列リクエスト構成】
Request A: slugs=["can-badge","acrylic-keychain"]（bottom-center画像・fal.aiなし）
  → Worker が即時SUZURI登録 → 即返答
  → R2 meta.json に products を書き込む（r2Id付きで呼び出す）

Request B: slugs=["t-shirt","sticker"]（bottom-right画像・fal.ai挑戦）
  → Worker がctx.waitUntil()でバックグラウンド処理
  → 先にHTTPレスポンスを返す（"queued": true）
  → バックグラウンドでfal.ai → SUZURI登録 → R2 meta.jsonにproductsをマージ

フロントエンドのポーリング:
  → t-shirt/stickerボタンが押されたら GET /image/{r2Id} でmeta.jsonを確認
  → products に t-shirt エントリがあれば SUZURI URLへ遷移
  → なければトースト「まもなく準備完了です。しばらくお待ちください」
```
...
| 懸念 | 内容 | 状態 |
| --- | --- | --- |
| ctx.waitUntil() wall-clock上限 | fal.aiがレスポンス返却後も完了できるかCloudflare仕様が不明確（I/OはCPU時間に計上されないため動く可能性あり） | 要検証 |
| products のマージ書き込み | `updateMetaInR2`は`{ ...existing, ...updates }`でproductsを上書きするため、2グループが別々に更新するとどちらかのproductsが消える | **要修正**（スラッグ単位でマージするロジックが必要） |
| ユーザーが遷移前にタブを閉じた場合 | ctx.waitUntil()タスクはサーバー側で継続するため問題なし | 問題なし |
| SUZURIマテリアルID管理 | 2グループが別々のmaterialIdを持つ。現状もそうなので変更不要 | 問題なし |

**必要な変更ファイル:**
- `worker/index.js`: `/suzuri-create`をctx.waitUntil()対応に改修
- `worker/r2-storage.js`: `updateMetaInR2`のproductsマージロジック追加
- `frontend/index.html`: t-shirt+stickerボタンのポーリングロジック追加
- `scripts/test-bot.mjs`: ポーリングシナリオのテスト追加
