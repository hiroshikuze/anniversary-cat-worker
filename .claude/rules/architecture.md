...
| 英語ページのテーマ表示（2026-05） | `?lang=en`時かつ`themeEn`/`descriptionEn`が存在する場合、テーマ名・説明文を英語で表示。取得できない場合は日本語にフォールバック |
...
| Umamiアナリティクス | `cloud.umami.is/script.js`でページビュー自動収集。共有ページ（`loadSharedImage()`成功時）は`window.umami?.track()`で明示的にページビューを送信（`replaceState`のみでは検知されないケースに対応）。SUZURIボタンクリックを`data-umami-event="suzuri-click"`＋`data-umami-event-slug={slug}`で計測 |
