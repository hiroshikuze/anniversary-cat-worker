...
- **毛柄**: 種ごとに代表色・模様を1〜2パターン定数化する。種を指定すれば外見はAIが補完するため過剰な指定は不要
  - 例: dog → "golden retriever" / "shiba inu" から選ぶ
  - 例: rabbit → "white fluffy" / "gray lop-eared" から選ぶ
- **性格**: 猫の5タイプをそのまま当てはめると違和感が出るため、動物固有の1行で表現する
  - 例: `"friendly and energetic, approaching the cat with enthusiasm"`

##### 猫ゲスト（No.7）似た年齢の猫

毛柄の選び方で関係性のニュアンスが変わる。確率比は実装時に決定。

| 毛柄の選び方 | 印象 | プロンプト例 |
| --- | --- | --- |
| 同系（タビー同士など） | 兄弟・血縁 | `"littermate with similar coat"` |
| 対照（主人公がオレンジ→ゲストが白黒等） | 友人・対比映え | `"best friend cat with contrasting coat"` |
| 完全ランダム独立 | 縁のある他人 | `"companion cat"` |
...
- **子猫側**:
  - 毛柄: 主人公と同系が血縁らしい（確率で揃えるか完全独立かは実装時に決定）
  - 性格: `"leaning forward with wide curious eyes"` 固定（子猫らしさ）
- **主人公側**:
  - `pickPersonality()` の結果に「保護者ポジション」の修飾を追加または上書き
...
```text
23:50 Cron（日〜木）:
  → Gemini × N回で翌日の記念日候補を収集
  → フィルタリング（google-search-fallback除外 等）
...
| 項目 | 選択肢 | 状態 |
| --- | --- | --- |
| 生成方式 | バッチ（1回で複数件）vs シングル（1件ずつ複数回） | **要実測** → `scripts/test-gemini-research-batch.mjs` |
| Cron時刻 | 前日23:50 vs 当日06:00 | 23:50案が有力（翌日日付確定済み）。失敗時の気づきやすさはデメリット |
| 候補生成件数 | 10件程度 | フィルター後の最低件数（例：3件未満でフォールバック）も要設定 |
| フィルタリング基準 | ①google-search-fallback除外（実装容易）②説明文に日付が含まれるか ③Gemini再確認（追加コスト） | 優先度順に①から導入 |
| 事前算出フィールド | theme / description / visualHint / kanjiChar / foodItem / sourceUrl | 猫ペルソナ・性格・感情は生成時ランダムのまま |
| プールのR2キー | `research-pool/YYYY-MM-DD`（既存の`bot/`・`user/`と分離） | 保持期間は1〜2日で十分 |
| `/research`レート制限 | プール方式ではR2読み取りのみのため制限の根拠が変わる | 実装時に見直し |
| ユーザーの「もう一度」体験 | 同日は同じプール内でランダム選択（テーマが変わる回数は候補数が上限） | 許容可と判断 |
| コスト増加 | 現行：ユーザーresearch呼び出しは都度課金 → 変更後：固定で1日N回 | バッチ実測後に試算 |

##### バッチ方式 vs シングル方式の検証

```bash
GEMINI_API_KEY=xxx node scripts/test-gemini-research-batch.mjs
```

**確認ポイント:**
- バッチ方式でグラウンディングチャンクが全候補で共有されるか（個別URL取得不可の懸念）
- シングル方式の`google-search-fallback`率（現行の品質ベースライン）
- 処理時間差（シングル並列化でどこまで短縮できるか）

**バッチ方式の既知リスク:** グラウンディングはレスポンス全体に1セットしか付かないため、候補ごとの個別sourceUrlが取れない可能性がある。その場合、フォールバック判定が「全件同じ」になりフィルタリング粒度が落ちる。

##### 実装タイミング

`test-gemini-research-batch.mjs`の実測結果を確認してから設計を確定し着手する。
...
- `kanjiChar`がnullまたは無効値の場合は `"🐾"` を使用（肉球フォールバック。offscreen Canvas + source-atop で `#aaaaaa` グレー化）
