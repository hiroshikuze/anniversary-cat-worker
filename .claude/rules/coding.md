...
- **`runBot()`（Cron Trigger実行パス）に処理を追加する際は、`Intl.Segmenter`・`new Date()`・正規表現等のコストがある処理を不要に重複実行していないか検討する**（2026-08追加。Workers Free プランのCron Trigger CPU時間上限は10msと非常に厳しいためBug#32参照）
...
- **`runBot()`実行パスに「重そうな処理」（画像デコード/エンコード・WASM呼び出し・大きめのJSON解析・ループを伴う文字列処理等）を新規に追加する際は、`performance.now()`ベースの計測チェックポイント（`console.log`で`[cpu] {処理名}: {ms}ms`形式）を併せて追加する**（2026-08追加。Bug#32のCPU時間分析の一環）
