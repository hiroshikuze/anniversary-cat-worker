...
- **Cron・HTTPエンドポイント問わず「重そうな処理」（画像デコード/エンコード・WASM呼び出し・大きめのJSON解析・ループを伴う文字列処理等）を新規に追加する際は、`performance.now()`ベースの計測チェックポイント（`console.log`で`[cpu] {処理名}: {ms}ms`形式）を併せて追加する**（2026-08追加。Bug#32のCPU時間分析の一環）
  ...
  - **既存のレート制限で呼び出し回数が有界な経路**（`/research`・`/generate`・`/suzuri-create`・Cron等）は、`console.log`に加えて`incrementCpuTimeKv()`でKV集計しCIログ・`/cpu-usage`から確認できるようにする。**レート制限のない高頻度経路**（`/image/:id`等）はKV書き込み1,000回/日の予算を圧迫するため`console.log`のみに留める（Workers KV Freeプランの書き込み上限。詳細は`architecture.md`の「CPU時間のステップ別記録」参照）
