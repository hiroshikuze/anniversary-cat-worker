...
- **Cron Trigger・HTTPエンドポイントを問わず、処理を追加する際は`Intl.Segmenter`・`new Date()`・正規表現等のコストがある処理を不要に重複実行していないか検討する**（2026-08追加。Workers Free プランのCPU時間上限はCron/HTTPいずれも10msと非常に厳しいためBug#32参照）
  - 同じ入力に対して同じ計算を複数箇所で行っていないか（呼び出し元で計算済みの値を引数で渡せないか）
  - ループや条件分岐の中で、本来スキップできるはずの重い処理（`Intl.Segmenter`によるフルスキャン等）を毎回実行していないか。文字列長（`.length`）等の軽量な指標で先にfast pathを判定できないか
- **Cron・HTTPエンドポイント問わず「重そうな処理」（画像デコード/エンコード・WASM呼び出し・大きめのJSON解析・ループを伴う文字列処理等）を新規に追加する際は、`performance.now()`ベースの計測チェックポイントを併せて追加する**（2026-08追加。Bug#32のCPU時間分析の一環）
  - 「重そうか」の判断に迷う場合は、ネットワーク待ちを含まない同期処理で、入力サイズが数百バイト〜数百KB以上になりうるものを目安にする
  - 一時的な調査用ではなく恒久的に残す前提で追加する（Workers Logsの無料枠は1日20万イベントまでで、このBotの実行頻度では十分に収まるため）
  - 計測を増やしすぎるとログ出力自体がCPU時間を消費するため、疑わしい箇所に絞って追加し、明らかに軽い処理（単純な文字列結合・`??`演算子等）には追加しない
  - **必ず`worker/index.js`の`recordCpuCheckpoint(step, ms, kv = null)`（`console.log`で`[cpu] {step}: {ms}ms`形式 + 任意でKV集計）経由で追加する**。`console.log`とKV集計を個別に書かない（重複コード防止）
  - **既存のレート制限で呼び出し回数が有界な経路**（`/research`・`/generate`・`/suzuri-create`・Cron等）は`recordCpuCheckpoint(step, ms, env?.RATE_KV)`のように`kv`を渡し、CIログ・`/cpu-usage`から確認できるようにする。**レート制限のない高頻度経路**（`/image/:id`等）は`kv`を省略する（`recordCpuCheckpoint(step, ms)`）。KV書き込み1,000回/日の予算を圧迫するため（Workers KV Freeプランの書き込み上限。詳細は`architecture.md`の「CPU時間のステップ別記録」参照）
