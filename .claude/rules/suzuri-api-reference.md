...
| メソッド | パス | 用途 |
| --- | --- | --- |
| `PUT` | `/api/v1/products/{productId}` | 既存商品（`materialId`とは別の個別商品ID）の`price`/`title`/`description`等を、マテリアルを削除・再作成せずに編集する |

**確認状況（2026-09・未検証）**: 公式ドキュメントへの直接アクセス（WebFetch）はsuzuri.jpのWAFで403のため確認できていない。上記はWebSearch経由でヒットしたサードパーティ情報（Zenn記事等）に基づく未確認情報であり、実際のcurl実行での動作確認も行っていない。**このプロジェクトのコードには未実装**（`worker/suzuri.js`に対応する関数なし）。使う場合は事前に実際のAPIレスポンスで仕様を確認すること（本ファイル冒頭の「原典」に倣い、確認できたら本セクションを更新する）。

背景: SUZURIマテリアル1件の価格を誤登録した際、削除→再作成すると`sampleUrl`（`materialId`ベース）が変わり既存の共有リンクが壊れるため、削除せず価格のみ修正する方法を調査した際に見つかった。
