...
- **修正**: `/suzuri-create`ハンドラーで、backTextureをR2に`${r2Id}/back.jpg`としてアップロードし、`${workerOrigin}/back/${r2Id}`というWorker URLをSUZURIに渡す。Worker URLはSUZURIから安定してアクセスできる（`/hires/:id`で動作確認済みの方式と同一）。`r2Id`がない場合はbase64フォールバック（現状維持）
...
- **教訓**:「タイムアウトを追加した」という確認は「外部fetchを持つ関数すべてを`grep -n "AbortSignal\|fetch("`でリストアップして照合する」まで行う。とくに「MastodonとBluesky両方に追加して」という指示のように複数対象がある場合は、実装後に全対象を機械的に確認する
...
- **ミス**: ドキュメント・テストを追加せずにコードを先に変更した。ユーザーから指摘されてはじめて気づいた
- **経緯**:「シンプルな属性追加だから」という判断でプロセスを省略した
- **教訓**: 変更の大小にかかわらず「Docs → Tests → Code」の順序は常に守る。テスト不要（DOM依存）の場合でも、その旨を先に説明してからコードに着手する
...
- **再発防止策（今セッション実装）**:
  - CI（`health-check.yml`）にgrep-basedフロントエンド機能存在チェック（11項目）を追加。問題のある`frontend/index.html`がpushされると即CIが失敗する
...
- **同じパターン**: 2026-03「SUZURI API招待制」・2026-04「Wikipedia調査でfetch403を確認なしに確定情報として報告」とまったく同じミス
...
### 2026-07 | 1件の観測から特定ワードを原因と断定しかけた（丸皿画像バグ・Bug#27）

- **状況**: ユーザーから「生成画像が丸皿風になった。SUZURI連携に悪影響」と報告を受け、`SEASONAL_FLOWERS`の蓮エントリのStyle行`"calm pond atmosphere"`（24エントリ中唯一Style行に具体的情景名詞を含む）を原因と断定し、その除去を主策とする修正プランを提示した
- **ミス**: 実際のプロンプト全文（Theme/Context/Setting）を確認せずに配色の一致（傍証）だけで因果関係を主張していた。ユーザーに「本当にそれが原因か、プロンプトはテーマ方向でまとまっていないか」と問われてはじめて、Theme/Context/Setting側には蓮・池を連想させる語が一切なく、内容面はテーマ通り正しく生成されていたことに気づいた
- **教訓**: 傍証（色の一致）と直接因果（特定の症状の原因）を混同しない。とくに外部APIで再現実験ができない環境（サンドボックスからGemini APIを叩けない）では、1件の観測結果だけで特定ワードを原因と断定せず、「原因が確定しなくても効果がある対策（今回はネガティブ指示によるDefense-in-depth）」を主策にし、疑わしいワードの変更は「一貫性改善」等の低リスクな位置づけに留めてドキュメントにも確度を明記する

---

### 2026-07 | テストファイルを新設・変更しても`npm test`から呼ばれているか確認していなかった（外部通信リトライ監査・Bug#28）

- **状況**: 外部通信リトライ監査でギャップ2（`worker/suzuri.js`のリトライ追加）の回帰テストを`scripts/test-suzuri.mjs`に追加しようとした
- **発見**: `scripts/test-suzuri.mjs`は`package.json`の`test`スクリプトにも`.github/workflows/health-check.yml`にも接続されておらず、書かれた当初から一度もCIで実行されていなかった。しかも単体実行すると、`worker/suzuri.js`側の`res.text()`移行（coding.md規約）にテストのモックが追随しておらず現に失敗する状態だった
- **教訓**: 新しいテストファイルを追加する、または既存のテストファイルに手を入れる際は、そのファイルが実際に`npm test`やCIワークフローから呼ばれているかを`grep`等で確認する。「テストファイルが存在する」ことと「テストが実行されている」ことは別であり、後者を確認しないと壊れたテストが放置されたまま気づかれない

---

### 2026-08 | Cloudflare Cronの曜日番号は標準Unix cronと異なる（1=日曜日）と知らずに誤った運用分析を報告

- **状況**:「今日am7時のcron分も見ていますか」と聞かれ、標準的なUnix cron（0または7=日曜日）の感覚で`0 22 * * 1-5`を解釈し、「今日は日曜日だから月〜金（1-5）に含まれず、Bot投稿Cronは発火していない」と分析して報告した
- **実態**: `wrangler.toml`のコメントに「Cloudflare Workersの曜日指定は1=日曜日のため、1-5=日〜木UTC=月〜金JST」と明記されていた。標準Unix cronとは異なる採番だったため、日曜UTC 22:00の発火はまさに月曜JST朝の投稿に対応しており、実際にはCronは正常に発火していた。`/meta/bot/{今日のJST日付}`を直接確認してはじめて投稿が存在することに気づいた
- **教訓**: 外部システム（Cloudflare等）のcron・スケジュール表記は、標準的な慣習と異なる場合がある。記憶や一般知識で解釈する前に、該当システムの設定ファイル（`wrangler.toml`のコメント等）を直接確認する。「発火したかどうか」のような検証可能な事実は、抽象的な計算で結論を出す前に、実際のデータ（今回は`/meta/bot/:id`エンドポイント）で確認してから報告する

### 2026-08 | 既存の計測パターンを新しいステップ名に複製した際、同じKVキーへの並行書き込みでレースコンディションを自己生成（generate-jsonParse追加時）

- **状況**: `generate`ステップの高い計測値の内訳を調べるため、同じ`handleGenerate()`内に`generate-jsonParse`という2つ目のCPU計測ステップを追加した。両方のPromiseを先に生成してから`ctx.waitUntil()`/`_deferOrAwait()`に渡す（並行実行を保つ）という、直前まで使っていた最適化パターンをそのまま複製した
- **ミス**: `generate`と`generate-jsonParse`はどちらも同じKVキー（`cpu-time:YYYY-MM-DD`）にGET→PUTで書き込む。並行実行するとread-modify-writeが競合し、後勝ちで一方の更新が失われる。「並行実行を保つ」という最適化は、書き込み先のKVキーが異なる場合にのみ安全という前提を見落としていた
- **発見経緯**: 事前にDocs→Tests→Codeの順でテストを先に書いていたため、コード実装直後の`npm test`でテストが即座に失敗し、レースコンディションを自分で作り込んだことにその場で気づけた
- **教訓**: 既存の並行実行パターンを新しい箇所に複製する際は、「そのパターンが安全だった理由」（今回は書き込み先キーが異なっていたこと）を確認してから適用する。同じ日次ドキュメント（同じKVキー）に複数のステップを追記する設計を追加する場合は、必ず直列化する。Docs→Tests→Codeの順序を守っていたおかげで、実装ミスをコード変更の直後に検出できた（テストを先に書く効能の実例）

### 2026-08 | fal.aiポーリング予算の拡大を「余裕がありそう」という推測ベースで提案しかけ、実測で覆した

- **状況**: fal.aiアップスケール未完了（base64フォールバック）のDiscord通知が2日連続で来ていた件をユーザーと調査し、`query-worker-logs.mjs`（新設）で該当requestIdのログを確認。3回×5秒ポーリングが`IN_QUEUE`のまま尽きていたことを突き止め、「`ctx.waitUntil()`の~28秒予算にはまだ余裕がありそう（推測: 現状25秒程度使用と見積もり）だからポーリング回数を増やせないか」と提案した
- **ユーザーからの指摘**:「ポーリング予算を伸ばしたら場合によって全体的にタイムアウトするリスクは大丈夫か」と問われ、実測せず見積もりだけで提案していたことに気づいた
- **実測（`query-worker-logs.mjs`の`--request-id`フィルターを追加し、`$metadata.requestId`で1回のWorker呼び出しの全ログ行を相関）**:
  - 失敗ケース（3回とも`IN_QUEUE`→base64フォールバック）: `bg開始`から`right グループ完了`まで**約20.3秒**
  - 成功ケース（3回目のポーリングで`COMPLETED`）: **約25.6秒**（28秒予算に対し残りmarginは約2.4秒）
- **発見**: 成功ケースの方が完了後の処理が重い（CDN fetch→R2保存→SUZURI登録で約9.7秒）。失敗ケースのbase64フォールバックはローカルの画像を使うだけなので約4.3秒で済む。この非対称性は事前の見積もりでは気づけなかった
- **結論**: ポーリング予算を伸ばすと、成功に近づいたケース（3回目でCOMPLETEDになるようなギリギリのタイミング）ほど後処理の重さで28秒予算を超過し、強制終了＝何も保存されない方向に倒れるリスクが実測より高いことが判明。当初の提案を撤回し、`ctx.waitUntil()`内のポーリング予算は現状維持と判断した（`CLAUDE.md`の「変えてはいけない設計判断」に追記済み）
- **教訓**: Cloudflare Workersのwall-clock予算のようなハードリミットに近い設計変更を提案する際は、「まだ余裕がありそう」という推測を提案の根拠にしない。ユーザーがリスクを問うた時点で「計測してから閾値を決めましょう」と提案を差し戻されたのは正しい判断で、実際に計測すると見積もりの前提（後処理は数秒程度で軽い）が誤っていた。本セッションで追加した`query-worker-logs.mjs`の`--request-id`フィルターは今後も同種の実測に再利用できる

### 2026-08 | `performance.now()`ベースのCPU計測が、I/Oを挟まない同期処理では常に0msを返すことを知らずに計測フェーズを設計した（生成後の自動トリミング）

- **状況**: Bot投稿の余白過多が再発した件で、Photonによる生成後の自動トリミング（`worker/image-utils.js` `autoCropImage()`）を追加した。Node.js環境の実測で約45〜60ms/回とWorkers Free CPU上限（10ms）を大きく超えることが分かったため、「まず`/generate`のみに適用し、`recordCpuCheckpoint()`で実際のCloudflare Workers上のCPU時間を計測してから`runBot()`への適用可否を判断する」という計測フェーズの設計にした
- **状況（続き）**: ユーザーが実際に`/generate`を2回実行後、`/cpu-usage`を確認したところ、`generate-autoCrop`ステップが常に`0.0ms`だった（エラーは出ておらず正常に実行されている）
- **原因**: Cloudflare WorkersはSpectre攻撃対策として、`performance.now()`（`Date.now()`も同様）が**I/Oが発生したときしか進まない**という仕様になっている（[公式ドキュメント](https://developers.cloudflare.com/workers/runtime-apis/performance/)で確認済み）。`autoCropImage()`はPhotonのデコード・縮小・crop・エンコードがすべて同期的なWASM/JS処理でI/Oを一切挟まないため、開始・終了の`performance.now()`が同じ値になり差分が必ず0になる。これは実装ミスではなく、この計測方法自体の構造的な限界
- **気づいた副次的事実**: `generate`ステップも過去のログで確認する限り同様に常に`0.0ms`前後だった可能性が高い（本セッションではじめて気づいた）。従来Bug#32で報告されていた`maxMs=94ms`・`maxMs=666ms`等の非ゼロ値は、計測区間内にfetch()等のI/Oが挟まっていたケース（そのI/O自体の待ち時間で`performance.now()`が進んだ）であり、純粋な同期CPU処理だけを区間に含む計測は原理的に常に0になる
...
- **原因**: `worker/index.js`にはこの問題（Geminiモデルの廃止）に対応するための`selectBestModel()`（Discovery APIでモデル一覧を取得しスコアリング・KV記憶・切替時Discord通知まで行う既存機構）がすでに存在するにもかかわらず、新規実装時にそれを使わず固定文字列を書いてしまった。`architecture.md`の「Geminiモデル管理」セクションを実装前に読み返していれば防げた
...
- **追記（実装抜けの棚卸し中に発見）**: 上記修正のPRレビュー後、ユーザーから「実装抜けがないか確認して」と依頼され棚卸しした結果、`selectBestModel()`自身の内部にも同種の問題が残っていることが判明した。Discovery API呼び出しが完全に失敗した場合の最終フォールバック（`_selectFromCandidates()`の候補0件時・`selectBestModel()`のcatch節）が`"gemini-2.5-flash-lite"`をハードコードしており、これも今回廃止が判明したモデルだった。動的スコアリングアルゴリズム自体は正しく動作する（Discovery APIが応答する限り廃止モデルは選ばれない）ため実害はほぼないが、「保険用のハードコード値」も見落としやすい箇所だと分かったため、`FALLBACK_TEXT_MODEL`という名前付き定数に切り出し1箇所で管理するよう修正した。**教訓の教訓**: 1つのハードコード値を見つけて直したら、同じファイル内に類似のハードコード値が他に残っていないか`grep`で確認する習慣が必要

### 2026-09 | visualHintの主役名詞がテーマを猫に擬人化し画像内に2匹目の猫顔が出現（Bug#33）

- **状況**:「草の日」テーマの生成画像で、メインの猫（白いラグドール）とは別に、草むらの中に猫の顔がもう1つ描かれているとユーザーから報告を受けた
- **原因**: 実際のプロンプト全文を確認したところ、`visualHint`が`cute green cat, lush grass field, ...`となっており、`handleResearch()`のvisualHint生成指示「主役となる名詞を1〜2語で先頭に抽出」にしたがってGeminiが「草」自体を「かわいい緑の猫」として擬人化していた。2026-07に未対応のまま残っていたバグ（食材が主役名詞になり猫に合成される＝半夏生でタコの足が生えた件）と同じ原因の類型で、対象が食材から植物に広がったケース
- **修正**: Bug#27と同じ「原因が確定していても対症療法を主策・原因対処を補助策とする」方針で両方実施
  1. `_buildGeminiPrompt()`に常時ネガティブ指示`Only the cat(s) described above should have a face, eyes, or expression. Do not depict grass, plants, flowers, food, or any other scenery element with a face, eyes, or anthropomorphized expression.`を追加（従来`eatingAction`時のみだった指示を汎用化・統合）
  2. `handleResearch()`のvisualHint生成指示に「テーマを猫や他の動物に擬人化しない」制約を追加
- **副次的な確認事項**: ユーザーが気にしていた「Discordのプロンプト表示が途中で途切れる」件は、`notifyDiscord()`の2,000文字上限による表示上の切り詰め（既存仕様）であり、実際にGemini APIへ送信されるプロンプト自体は途切れていないことを確認・説明した
- **教訓**: 2026-07に「設計の影響範囲が広いため別セッションで判断」として先送りしたバグは、症状の対象（今回は植物）を変えて再発した。先送りしたバグは`architecture.md`に記録するだけでなく、類似テーマ（動物・物・人物を主役名詞にする設計全般）で再発しうることを次回発生時にすぐ参照できる形にしておく

### 2026-09 | Bug#34修正で`onlyIf.etagMatches`にクォート付き`httpEtag`を渡し本番でR2条件付きPUTが常に失敗した

- **状況**: 前セッションがBug#34（`updateMetaInR2()`のロストアップデート）修正でR2条件付きPUTを導入した際、`onlyIf.etagMatches`に`obj.httpEtag`（クォート付き文字列）を渡した。実行検証はできず、Miniflare参照実装ソースの読解のみを根拠に「実行検証していない」旨を明記してマージした
- **ミス**: デプロイ翌日、`/suzuri-create`が`Conditional ETag should not be wrapped in quotes`エラーで複数回失敗しているとユーザーから報告を受けた。改めて`developers.cloudflare.com/r2/api/workers/workers-api-reference/`を直接fetchしたところ、`onlyIf.etagMatches`にはクォートなしの`etag`を使うべきと明記されており、前セッションの推測（`httpEtag`）は誤りだった。さらに、この誤りは`scripts/test-bot.mjs`の`makeMockBucket()`が本物のworkerd R2 APIの制約（クォート付き`etagMatches`を拒否する）を再現しておらず、モックがクォート付き同士の比較で平仄を合わせてしまっていたため、テストでは検知できなかった
- **教訓**:「実行検証できないので公式ドキュメント＋参照実装で判断し、その旨を明記する」という進め方自体は正しいが、**未検証の判断は本番投入後に実際のログ・エラーで裏が取れるまで暫定として扱う**必要がある。また、外部APIの制約をテストで守りたい場合、モックはテスト対象コードに合わせて書くのではなく、**実際のAPIが拒否する入力（今回ならクォート付きetagMatches）をモック側でも明示的に拒否するよう実装し、その拒否自体をテストで検証する**べきだった。詳細は`.claude/bugs-history.md`のBug#34追記参照

### 2026-09 | 新セッション開始時、既知の継続案件について`git log`でリモートの進捗を確認せずに調査を始めた

- **状況**: 前セッションでBug#34（SUZURI多重登録）を修正した直後にセッションが終了。新セッションで「suzuriについて重複登録があれば削除して」という依頼を受け、`.claude/revision_log.md`を読んだ直後、`git fetch`/`git log`で他セッションの進捗を確認しないまま本番R2メタのcurlチェックに着手した
- **経緯**: 調査を進める途中でユーザーから「最新のリポジトリからpullして」と指示され、そこではじめて別セッションが`httpEtag`誤り（自分が入れたバグ）を発見・修正し、さらにロールバック機構まで追加していたことに気づいた。ユーザーの指示がなければ、すでに修正済みの`httpEtag`バグを前提に古い状態のまま調査・報告していた可能性がある
- **教訓**: `updateMetaInR2()`のように複数セッションにまたがって継続中と分かっている案件（`CLAUDE.md`の「変えてはいけない設計判断」に載っているような直近の変更）を再開するときは、`.claude/revision_log.md`を読むのと同じタイミングで`git fetch && git log HEAD..origin/main --oneline`を実行し、自分の知らない間に他セッションが進めていないか確認する習慣をセッション冒頭のルーチンに含める。とくに本番障害系の修正は複数セッションにまたがって続くことがあるため、「前回の自分の作業が最新」という前提を置かない

### 2026-09 | 生のsatoriパッケージをWorkers向けに実装したが、実際にはビルド自体が不可能だった（wrangler --dry-runで発覚）

- **状況**: 月替わり壁紙機能でPhotonのdraw_text()の代替として、WebSearchで調査した「Cloudflare Workers上でのSatori+resvg-wasm」パターン（OGP画像生成等で広く使われる組み合わせという情報）に基づき、生の`satori`パッケージ（`satori/standalone`）を直接importする実装を書き、ユニットテスト（モック経由）も通してドキュメント化・コミットまで済ませた
- **ミス**: 実装が「動く」ことをユニットテストの成功だけで確認しており、実際にCloudflare Workers向けにビルドが通るかを一度も検証していなかった。ユーザーから「途中の方針転換で無駄なコード・リンク切れがないか確認して」と棚卸しを依頼された際、はじめて`npx wrangler deploy --dry-run`を実行したところ`Could not resolve "fs"`でビルド自体が失敗した（`satori`が依存するharfbuzzjs＝Emscripten生成JSがNode専用の`require("fs")`を静的に含むため）。デプロイ済みだったら本番障害になっていた可能性があるミスを、たまたま棚卸しの流れで発見できた
- **教訓**: 新しいnpm依存（とくにWASM・ネイティブバインディングを含むもの）をCloudflare Workers向けに追加する際は、「モックでのユニットテストが通ること」と「実際にWorkers向けにビルドが通ること」は別問題であり、**ユニットテストが通った時点で満足せず、`npx wrangler deploy --dry-run --outdir=<tmp>`を必ず一度実行してビルド・バンドルサイズを確認する**。このコマンドはAPIキー不要でローカル実行でき、ドキュメント調査（WebSearch等）で「動くはず」と判断した設計を実地検証する最も安価な手段。`CLAUDE.md`の「テスト制約」に恒久ルールとして追記済み

### 2026-09 | 新規Cron Trigger追加時、対象Worker単体の本数しか見ておらず、Cloudflareアカウント全体の上限超過でマージ後のデプロイが失敗した

- **状況**: 月替わり壁紙機能の月末チェック用に、`wrangler.toml`の`crons`配列に新しいCron式（`"0 3 * * *"`）を追加する設計でPRを作成・マージした。このWorker単体では3本→4本になるだけで、Workers Freeの上限（5本）に収まると判断していた
...
### 2026-09 | resvg.wasm修正1回目のデプロイ後、`ensureResvg()`の真偽値フラグによる二重初期化防止が並行呼び出しの競合に対して安全でなかった

- **状況**: Bug#36（resvg.wasmの実行時fetch→静的importへの修正）をデプロイし、ユーザーに`POST /monthly-wallpaper/regenerate`の再実行を依頼したところ、依然として`composited: false`のままだった
- **ミス**: `ensureResvg()`の二重初期化防止が`if (_resvgReady) return;`という単純な真偽値チェックのみで、`compositeMonthlyWallpaper()`が`Promise.all()`で並行実行する2つの呼び出し（カレンダー版・署名版）が両方とも「まだ初期化されていない」と読んでから初期化処理に入ってしまう競合（double-checked lockingの典型的な欠陥）を見落としていた。この構造上の欠陥はR2実行時fetch版のコードにも最初から存在していたが、その手前で毎回「動的コード生成禁止」エラーが先に発生していたため一度も露見していなかった
- **教訓**: 1つ目の修正をデプロイして実機検証が依然失敗した場合、「まだ同じ問題が直っていないだけ」と決めつけず、`query-worker-logs`等で改めて実ログを取得し**エラーメッセージ自体が変わっていないか**を必ず確認する。今回はメッセージが`Wasm code generation disallowed`から`Already initialized`に変わっていたことに気づき、別種の問題（並行処理の競合）だと判断できた。また、一度きりの初期化APIを扱う際に「真偽値フラグ＋早期return」で二重実行を防ごうとする設計は非同期処理下では原理的に不十分であり、常に「進行中のPromiseを共有するシングルフライトパターン」を使う。詳細は`.claude/bugs-history.md`のBug#36追記参照

### 2026-09 | カレンダーなし版センタリング修正（ズームイン方式）が追加のフルリサイズでCPU予算を超過させWorkerごと強制終了された

- **状況**: Bug#37（月替わり壁紙の月名バッジ欠落・カレンダーなし版センタリング不足）の修正をデプロイし、ユーザーに`POST /monthly-wallpaper/regenerate`の再実行を依頼したところ、`error 1102`（Cloudflare Workersの強制終了）が返りDiscord通知も届かなかった
- **ミス**: カレンダーなし版センタリングの修正案（検出領域を切り出して`coverCrop()`で1080×1920へズームイン再フィットする）を設計する際、既存のSatori×2＋resvg×2＋Photon合成×2という処理パスが既にCPU負荷の高い経路であることを踏まえず、追加のフル解像度Lanczos3リサイズを気軽に1回上乗せしてしまった。`npm test`・`wrangler deploy --dry-run`はどちらもこの種の「実行時CPU予算超過」を検知できない（ビルド・モックロジックの正しさしか確認できない）ため、実機で初めて発覚した
- **教訓**: CPU負荷がすでに高い処理パス（月替わり壁紙のようにWASMベースのSatori/resvg描画を含む経路）に機能を追加する際は、「正しく動くか」だけでなく「既存処理に対してどれだけCPUコストを上乗せするか」を設計段階で意識する。とくに`resize()`（Lanczos3等の高品質フィルター）は`crop()`単体より大幅にコストが高いと想定されるため、同じ視覚目的を達成できるなら追加のリサイズを避け、既に計算済みのスケール済み画像を再利用する（クロップ窓をずらすだけ）方式を優先する。また、`error 1102`はJS例外ではなく基盤側の強制終了であり、`query-worker-logs`で見たときに「特定のログ行の直後で、後続のtry/catchのログすら出ないまま途切れる」パターンが典型的な兆候になる。詳細は`.claude/bugs-history.md`のBug#37追記参照

### 2026-09 | 「どう思うか」という質問に対し、Plan提示・承認を経ずいきなりDocs→Tests→Codeで実装を進めてしまった（Bug#38対応時）

- **状況**: ユーザーから「Geminiの助けも借りて以下を考えましたがどうですか？」という、意見を求める形の質問でスマホ実機のセーフエリア調整案（数値仕様付き）を受け取った。CLAUDE.mdルール1（Plan Mode Default）は3ステップ以上のタスクは実装前に計画を提示し承認を得ることを定めているにもかかわらず、提示された数値仕様をそのまま実装可能な仕様と解釈し、Plan提示・承認のステップを飛ばしてDocs→Tests→Codeを直接開始してコミット・pushまで完了させてしまった
- **ミス**: (1) 「どうですか？」という意見を求める文言は、実装の承認ではなく壁打ち・検討の依頼である可能性が高いにもかかわらず、それを区別せず実装着手のシグナルとして扱った。(2) レイアウト・見た目に関わる変更は、実際にCloudflare Workers実機でしか最終確認できない（`generate-autoCrop`と同様、ローカルでは`performance.now()`もWASM描画結果も検証できない）にもかかわらず、数値の妥当性を検証する簡易プレビュー（モックアップ画像等）を一切用意せず、ユニットテストの数値アサーションだけで「テスト完了」とみなして次のフェーズに進んでしまった
- **教訓**: ユーザーが「どう思うか」「どうですか」を明示的に尋ねている場合、それが実装済みのタスクの延長であっても一旦立ち止まり、実装着手前にPlanとして提示し承認を得る（ルール1を厳守する）。とくに視覚的なレイアウト調整は、Cloudflare Workers実機でしか最終検証できない領域であっても、ローカルで可能な範囲（PIL等での模式図・座標の可視化）でのプレビューを用意し、ユーザーが数値の意図した見た目を確認できるようにしてから実装に進む。「テストで数値をアサートできる」ことと「ユーザーが見た目を承認できる」ことは別物であり、後者が必要なタスクでは前者だけでTestsフェーズ完了と扱わない
- **再発防止策**: 上記の教訓を「気をつける」で終わらせず、`CLAUDE.md`ルール1に「着手判定基準」（意見を求める文言リスト・承認語を受け取るまでDocsフェーズへ進まない）、ルール7に「見た目・レイアウトに関わるタスクの追加ゲート」（Plan提示時に模式図を用意し事前合意を得る）として恒久ルール化した。次回以降は`CLAUDE.md`本文の該当ルールを直接参照して判断する
