...
- **状況**: eating action機能の実装で「ドキュメントを更新し、テスト可能なら追加した上でコーディング開始してください」と指示された
...
- **防止策**: TodoWriteでタスクを並べる際、必ず「ドキュメント → テスト → コード」の順にする
...
### 2026-05 | Docs→Tests→Code順序違反（Mastodon英語CTA語順修正）

- **状況**: ユーザーの文法確認に答えて「こちらが自然」と説明した直後に、コード・ドキュメントを先に変更してしまった
- **ミス**: ユーザーが「関連するドキュメントとテストを先に」と返す前にコードを実装。テストは後追いになった
- **原因**: 「軽微な1行修正」という判断でプロセスを省略した
- **教訓**: ユーザーの承認を得た瞬間に手が動いてしまう。承認後も「Docs→Tests→Code」の順序を確認してから着手する。特に会話の流れで「この変更でよいか？」→「はい」のパターンは要注意
