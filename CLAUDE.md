...
### 4. Subagent Strategy & Model Selection

リサーチ・分析はサブエージェントに委譲し、メインコンテキストを保全する。委譲時はタスクの複雑さに応じてモデルを使い分ける。

| モデル | 向いているタスク |
| --- | --- |
| Haiku | 定型的な検索・分類・フォーマット変換など、速度・コスト最優先のタスク |
| Sonnet | 通常のコーディング・バグ修正・中程度の設計判断 |
| Opus | アーキテクチャ設計・複雑な多段階推論・高い精度が必須な判断 |

メインセッション自身のモデルは自動切替しない（`/model`等のユーザー操作が必要なため）。ただし非自明なタスクに着手する前に、現在のモデルがタスクの複雑さ・速度/コスト要件に見合っているかを一言確認し、切り替えた方が良さそうな場合はユーザーに提案する。
...
実装タスクに着手する前に**必ずTodoWriteで手順を列挙する**。その際の順序は以下を厳守:

1. ドキュメント更新（仕様を先に確定する）
2. テスト追加（テスト不要の場合はその旨を明記）
3. コード変更

TodoWriteを列挙する前にルール1のPlan（計画提示・承認）を完了していること。フロー全体は **Plan → Docs → Tests → Code**。
...
- **Bot**: `@nyanmusu.bsky.social`（毎平日7:00 JST・2026-05-01より変更、記念日画像を自動投稿）
...
```text
Cron（月〜金 7:00 JST・2026-05-01より変更）→ scheduled()
  → runBot() → handleResearch() → handleGenerate()
  → saveToR2()（products:[]で保存）
  → Bluesky + Mastodon 並列投稿（Promise.allSettled・どちらか失敗しても継続）
  → Discord通知（両プラットフォームの成否を通知）
...
**`npx wrangler deploy --dry-run --outdir=<tmp>`でビルド検証・バンドルサイズ計測が可能（2026-09発見）**: 実際にはデプロイせず、Cloudflare Workers向けのビルド（esbuild）だけをローカルで実行できる。外部APIキー不要。新しいnpm依存（WASM・ネイティブバインディング等）を追加した際は、`npm test`だけでは検出できないビルド時エラー（Node専用API依存の混入等）とgzip後バンドルサイズ（Workers Freeプランは3MB上限）をこのコマンドで確認すること。実例: `satori`パッケージ導入時にharfbuzzjs依存の`require("fs")`がビルドを壊すことをこのコマンドで検出し、`@cf-wasm/satori`への切替で解消した（`.claude/rules/architecture.md`の「カレンダー・月名の合成」参照）。**ただし実行時のWASM初期化・動作まではローカルで検証できない**（Cloudflare Workers実機でのみ確認可能な点は変わらない）。
